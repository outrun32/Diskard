...
import { Info, LoaderCircle, Zap } from "lucide-react";
...
export function LaunchCheckPage() {
  ...
  const readyProfiles=profiles.data?.filter(p=>p.config.adapter_options.deployment_status!=="cached")??[];
  const cachedProfiles=profiles.data?.filter(p=>p.config.adapter_options.deployment_status==="cached")??[];
