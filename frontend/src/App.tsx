...
function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className={cn("app-frame",collapsed&&"app-sidebar-collapsed")}>
    ...
    <main id="main-content" tabIndex={-1} className="main-shell">
      <header className="topbar"><div/><div className="topbar-actions"><span className="language-label">{t("language")}</span><div className="language-switch" role="group" aria-label={t("language")}><button className={language==="en"?"active":""} onClick={()=>setLanguage("en")}>EN</button><button className={language==="ru"?"active":""} onClick={()=>setLanguage("ru")}>RU</button></div></div></header>
...
function MemoryInspector({ event, expanded, onExpand }: { event?: TraceEvent; expanded: boolean; onExpand: () => void }) {
  ...
  return <section className={cn("memory-pane", expanded && "memory-pane-expanded")}><div className="pane-header"><div><span className="eyebrow">Event inspector</span><h2>Memory / evidence</h2></div><IconButton label={expanded ? "Collapse inspector" : "Expand inspector"} onClick={onExpand}>{expanded ? <PanelRightClose size={16} /> : <PanelRightOpen size={16} />}</IconButton></div>{event ? <div className="inspector-content"><div className="inspector-selection"><span className="selection-bar-line" /><div><strong>#{String(event.sequence).padStart(2, "0")} · {event.operation ?? event.kind}</strong></div></div>{(event.content||event.response||event.detail)&&<div className="message-detail"><h3>Event content</h3>{event.content&&<><h4>Input</h4><pre dir="auto">{event.content}</pre></>}{event.response&&<><h4>Response</h4><pre dir="auto">{event.response}</pre></>}{event.detail&&<pre dir="auto">{event.detail}</pre>}<CopyButton text={[event.content,event.response,event.detail].filter(Boolean).join("\n\n")}/></div>}{memory ? <MemoryCard memory={memory} /> : <div className="inspector-empty"><div className="empty-dot"><EyeIcon /></div><h3>No linked evidence</h3><p>The backend did not provide a memory change or evidence link for this event. This does not prove that no change occurred.</p></div>}<div className="inspector-source"><div className="inspector-label"><span>Source event</span><code>{event.id}</code></div><div className="inspector-label"><span>Observed at</span><code>{formatFullDate(event.timestamp)}</code></div>{event.evidenceIds?.length ? <div className="evidence-links"><span>Evidence IDs</span><div>{event.evidenceIds.map((id) => <code key={id}>{id}</code>)}</div></div> : null}</div><details className="raw-disclosure"><summary><Braces size={14} /> Redacted raw event <ChevronDown size={14} /></summary><pre>{JSON.stringify(event.raw ?? event, null, 2)}</pre></details></div> : <div className="inspector-empty inspector-empty-start"><div className="empty-dot"><Database size={18} /></div><h3>Select an event</h3><p>Linked memory and evidence appear here when you select a trace row.</p></div>}</section>;
}
...
function TracePane({events,selectedId,onSelect}:{events:TraceEvent[];selectedId?:string;onSelect:(event:TraceEvent)=>void}){
 ...
 return <section className="trace-pane"><div className="pane-header"><h2>Trace</h2><span>{visible.length} / {events.length}</span></div><div className="trace-toolbar"><div className="trace-filter"><Search size={14}/><input aria-label="Search events" placeholder="Search messages" value={search} onChange={e=>setSearch(e.target.value)}/></div><select aria-label="Event actor" value={actor} onChange={e=>setActor(e.target.value)}><option value="">All actors</option>{[...new Set(events.map(e=>e.actor).filter(Boolean))].map(a=><option key={a}>{a}</option>)}</select><select aria-label="Event type" value={kind} onChange={e=>setKind(e.target.value)}><option value="">All types</option>{[...new Set(events.map(e=>e.kind))].map(k=><option key={k}>{k}</option>)}</select></div>
...
function InspectorDialog({event,onClose}:{event?:TraceEvent;onClose:()=>void}){
 ...
 return <dialog ref={ref} className="inspector-dialog" onClose={onClose}><button autoFocus className="button button-secondary" onClick={onClose}>Close inspector</button><MemoryInspector event={event} expanded={false} onExpand={onClose}/></dialog>;
}
