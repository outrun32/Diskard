...
import { DEMO_MODE, cancelRun, deleteCheck, getRun, listRunsPage, renameCheck, rerunRun, drainEvents, saveReport, request } from "./api";
  ...
  return <div className={cn("app-frame",collapsed&&"app-sidebar-collapsed")}>
    ...
    <aside className={cn("sidebar",collapsed&&"sidebar-collapsed")}>
      ...
      <div className="sidebar-bottom">
        {!collapsed && <div className="sidebar-concepts"><a href="https://github.com/outrun32/Diskard" target="_blank" rel="noreferrer" className="sidebar-github-link"><Github size={14}/> GitHub</a><a href="https://github.com/outrun32/Diskard/blob/feat/generic-chat-probe-adapter/docs/showcase/README.md" target="_blank" rel="noreferrer">Скрины и GIF (artifacts)</a><span className="nav-label">🐢 Черепашки атакуют</span><a href="/concepts/soc-dashboard.html" target="_blank" rel="noreferrer">SOC dashboard</a><a href="/concepts/attack-graph.html" target="_blank" rel="noreferrer">Attack graph</a><a href="/concepts/attack-graph-v2.html" target="_blank" rel="noreferrer">Attack graph v2</a></div>}
...
function RunsPage() {
  ...
  return <div>
    <PageHeader title={t("runs")} description="Review full assessments and open their traces."><Button variant="primary" onClick={() => navigate("/runs/new")}><Plus size={16} /> {t("newRun")}</Button></PageHeader>
...
function RunHeader({ run, onCancel, onRerun, isRerunning }: { run: RunDetail; onCancel: () => void; onRerun: () => void; isRerunning: boolean }) {
  ...
  const headingTarget = run.checkId ? `/checks/${encodeURIComponent(run.checkId)}` : `/runs/${encodeURIComponent(run.id)}/trace`;
...
function TracePane({events,selectedId,onSelect,live,chatOnly=false,searchTerm,onSearchChange}:{events:TraceEvent[];selectedId?:string;onSelect:(event:TraceEvent)=>void;live:boolean;chatOnly?:boolean;searchTerm?:string;onSearchChange?:(value:string)=>void}){
 ...
 const chatEvents=messageEvents.filter((event,index,items)=>{
  if(event.status!=="started")return true;
  const completion=items.slice(index+1).find(candidate=>{
   if(candidate.status!=="completed"||candidate.content!==event.content||!candidate.response)return false;
   const sameActor=!event.actor||!candidate.actor||event.actor===candidate.actor;
   const sameOperation=candidate.operation===event.operation;
   const sameSession=Boolean(event.session&&candidate.session&&event.session===candidate.session);
   return sameOperation||(sameActor&&sameSession);
  });
  return !completion;
 });
 ...
 <div className="trace-footer"><span>{chatOnly?"Turns":"Stored cursor"}: {chatOnly?source.length:events.at(-1)?.sequence??0}</span></div></section>;
}
...
function keyEventTone(event: TraceEvent): "danger" | "warning" | "neutral" {
  ...
  const verdict = String(raw.verdict ?? data.verdict ?? data.check_status ?? "").toLowerCase();
  const vulnerabilityFound = event.kind === "evidence" && (verdict === "vulnerable" || verdict === "observed" || verdict === "fail" || verdict === "failed" || details.leaked_vulnerable === true || details.leaked_in_vulnerable_mode === true || details.cross_identity === true);
  if (vulnerabilityFound || event.kind === "error" || event.status === "failed" || ["removed", "unavailable"].includes(event.memory?.change ?? "")) return "danger";
...
function PlaybackControls({events,index,onChange}:{events:TraceEvent[];index:number;onChange:(n:number)=>void}) {
 ...
 const lastIndex=Math.max(0,events.length-1),atEnd=index>=lastIndex;
 ...
 const nextIsStop=index+1===lastIndex;
 useEffect(()=>{if(!playing||atEnd)return;const timer=window.setTimeout(()=>{onChange(index+1);if(nextIsStop)setPlaying(false);},delay);return()=>clearTimeout(timer);},[playing,atEnd,index,delay,onChange,nextIsStop]);
 useEffect(()=>{if(atEnd)setPlaying(false);},[atEnd]);
 return <div className="playback-bar"><div className="playback-title"><History size={15}/><span>Recorded playback</span><span data-testid="playback-readonly" className="playback-readonly">Read only · target is not called</span></div><div className="playback-controls"><IconButton label="First" disabled={!events.length} onClick={()=>{setPlaying(false);onChange(0);}}><RotateCcw size={15}/></IconButton><IconButton label="Previous event" disabled={index<=0} onClick={()=>{setPlaying(false);onChange(index-1);}}><ArrowLeft size={15}/></IconButton><Button disabled={!events.length} onClick={()=>{if(atEnd)onChange(0);setPlaying(!playing);}}>{playing?"Pause":"Play"}</Button><IconButton label="Next event" disabled={atEnd} onClick={()=>{setPlaying(false);onChange(index+1);}}><ArrowRight size={15}/></IconButton><label>Speed<select value={speed} onChange={e=>setSpeed(Number(e.target.value))}>{[0.5,1,2].map(v=><option key={v} value={v}>{v}×</option>)}</select></label></div><label className="playback-range"><span>{events.length?index+1:0} / {events.length}</span><span className="playback-track"><span aria-hidden="true" className="playback-track-rail"/><input aria-label="Playback position" disabled={!events.length} type="range" min={0} max={lastIndex} value={Math.min(index,lastIndex)} onChange={e=>{setPlaying(false);onChange(Number(e.target.value));}}/>{events.map((event,eventIndex)=>event.kind==="message"&&!event.memory&&!event.evidenceIds?.length?null:<button key={event.id} type="button" className={cn("playback-marker",`playback-marker-${keyEventTone(event)}`,eventIndex===index&&"playback-marker-current")} style={{left:`${lastIndex?eventIndex/lastIndex*100:0}%`}} title={`${event.operation??event.kind} · #${event.sequence}`} aria-label={`Jump to ${event.operation??event.kind}, event ${event.sequence}`} onClick={()=>{setPlaying(false);onChange(eventIndex);}}/>)}</span><span className="playback-hint">Click a pin to jump</span></label>{sourceGap>3000&&<small>Pause compressed to 3 seconds; original timestamps are preserved.</small>}</div>;
}
...
function TraceViewInner({run,events}:{run:RunDetail;events:TraceEvent[]}){
 ...
 const cursor=Number(params.get("event")),requestedIndex=Number.isFinite(cursor)?events.findIndex(e=>e.sequence===cursor):-1,lastIndex=Math.max(0,events.length-1),rawIndex=requestedIndex>=0?requestedIndex:lastIndex,index=playback?Math.min(rawIndex,lastIndex):rawIndex;
 ...
 const toggle=()=>{const next=new URLSearchParams(params);if(playback){next.delete("mode");if(selected)next.set("event",String(selected.sequence));}else{next.set("mode","playback");if(events[0])next.set("event",String(events[0].sequence));}setParams(next,{replace:true});};
...
function RunDetailPage({ mode }: { mode: "trace" | "results" | "config" }) {
  ...
  return <div><RunHeader run={runWithEvents} onCancel={() => {if(!cancel.isPending)cancel.mutate();}} onRerun={() => {if(!rerun.isPending)rerun.mutate();}} isRerunning={rerun.isPending} />{(rerun.isError||cancel.isError)&&<Failure error={rerun.error??cancel.error}/>}<p className="rerun-note">{run.replay.reason}</p><RunTabs runId={id} mode={mode} />{mode==="trace"&&<>{eventQuery.isPending&&<p role="status">Loading events…</p>}{eventQuery.isError&&<Failure error={eventQuery.error} retry={()=>eventQuery.refetch()}/>}<TraceView key={id} run={runWithEvents} events={events}/></>}{mode === "results" && <ResultsView run={runWithEvents} />}{mode === "config" && <ConfigView run={runWithEvents} />}</div>;
}
...
function ReportsPage() {
  ...
  const reports = (query.data ?? []).filter((run) => !run.demo && !run.checkId && (run.status === "completed" || run.status === "failed" || run.status === "interrupted" || run.status === "cancelled" || run.status === "imported"));
