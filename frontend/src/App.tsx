...
import { DEMO_MODE, cancelRun, deleteCheck, deleteRun, getRun, listRunsPage, rerunRun, drainEvents, saveReport, request } from "./api";
...
function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className={cn("app-frame",collapsed&&"app-sidebar-collapsed")}>
    ...
    <aside className={cn("sidebar",collapsed&&"sidebar-collapsed")}>
      <div className="brand"><div className="brand-mark"><img src="/diskard-turtle-logo.png" alt="Diskard logo" /></div><div><div className="brand-name">DISKARD</div><div className="brand-subtitle">CONSOLE</div></div></div>
...
function RunsPage() {
  ...
  const checksQuery=useQuery({queryKey:["checks","runs-page"],queryFn:({signal})=>request("/api/v1/checks?limit=20&offset=0",{signal}) as Promise<{items:Array<{id:string;profile_id:string;profile_version:number;attacks:string[];created_at:string;status?:string;active_run_id?:string}>;total:number}>,refetchInterval:2500});
  const cache=useQueryClient();
  ...
  const removeCheck=useMutation({mutationFn:(id:string)=>deleteCheck(id),onSuccess:()=>{void cache.invalidateQueries({queryKey:["checks"]});void cache.invalidateQueries({queryKey:["runs"]});void cache.invalidateQueries({queryKey:["overview"]});void cache.invalidateQueries({queryKey:["reports"]});}});
  const deleteError=remove.error??removeCheck.error;
  ...
  return <div>
    ...
    <section className="runs-group-section"><div className="section-heading"><h2>{t("fullRuns")}</h2></div>{checksQuery.isPending?<p>{t("checksLoading")}</p>:checksQuery.isError?<DataError error={checksQuery.error} onRetry={()=>checksQuery.refetch()}/>:checksQuery.data.items.length?<div className="overview-results">{checksQuery.data.items.map(check=>{const live=activeStatus(check.status??"");const href=live&&check.active_run_id?`/runs/${encodeURIComponent(check.active_run_id)}/trace?check=${encodeURIComponent(check.id)}`:`/checks/${check.id}`;return <div className={cn("overview-run",live&&"overview-run-active")} key={check.id}><Link className="overview-run-main" to={href}><div>{live&&<span className="run-live-mark"><span/>In progress</span>}<strong>{check.profile_id}</strong><span>{check.attacks.length} {t("scenarios")}</span></div><span>{new Date(check.created_at).toLocaleString(language==="ru"?"ru-RU":"en-GB")}</span><ArrowRight size={16}/></Link><Button variant="danger" disabled={removeCheck.isPending} onClick={()=>{if(window.confirm("Delete this full attack and its runs?"))removeCheck.mutate(check.id);}}>Delete</Button></div>})}</div>:<p className="muted-copy">{t("noChecks")}</p>}</section>
    ...
    <section className="section-block">
      ...
      {(remove.isError||removeCheck.isError)&&<p role="alert" className="form-error">{deleteError instanceof Error?deleteError.message:"Could not delete run"}</p>}
